...
// const WebTorrent = require('webtorrent'); // Removed to fix ESM compatibility
...
/**
 * Returns the WebTorrent client, initializing it if necessary via dynamic import (ESM compatibility)
 */
...
async function downloadTorrent(magnet, ctx) {
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
            wt.add(magnet, { path: DOWNLOAD_DIR }, (torrent) => {
        ...
        ctx.telegram.editMessageText(ctx.chat.id, statusMsg.message_id, null, `❌ *Torrent Initialization Failed:* ${error.message}`).catch(() => { });
        throw error;
    }
}
